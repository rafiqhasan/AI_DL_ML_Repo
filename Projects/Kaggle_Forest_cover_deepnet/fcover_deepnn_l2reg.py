...
l2_regularization_penalty = 0.01    #L2 Bias constant BETA
...
#Calculate L2 regularization on WEIGHTS and NOT BIASES
l2_loss =   l2_regularization_penalty * (  tf.nn.l2_loss(W1) + 
                                           tf.nn.l2_loss(W2) +
                                           tf.nn.l2_loss(W3) +
                                           tf.nn.l2_loss(W4) +
                                           tf.nn.l2_loss(W5))
...
loss    =   tf.reduce_mean(entropy) + l2_loss
...
with tf.Session() as sess:
    ...
    for i in range(n_epochs):   #Train the model n_epochs times
        ...
        for b in range(n_batches):
            ...
            o_,a_,l_,cp = sess.run([optimizer,accuracy,loss,correct_prediction], feed_dict={X: X_batch, Y: Y_batch_enc, learn_rate: lr })
            #print(l_)
        ...
        acc_,lt_     = sess.run([accuracy,loss], feed_dict={X: Xt_data, Y: Yt_batch_enc})
